...
class supplierClass:
    def __init__(self,root):
        ...
        #======Content========================
        #=========row -1=================
        lbl_supplier_invoice=Label(self.root,text="Envoice No",font=("goudy old style",15),bg="white").place(x=50,y=80)
        ...
        self.supplierTable.heading("invoice",text="Envoice No.")
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                messagebox.showerror("Error","Envoice must be required",parent=self.root)   
            else:
                cur.execute("select * from supplier where invoice=?",(self.var_sup_invoice.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Envoice no. alredy assigned ,try diffrent",parent=self.root)
                else:
            ...
            if self.var_sup_invoice.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        ...
                        con.commit()
                        messagebox.showerror("Delete","Supplier deleted successfully",parent=self.root)
                        self.clear()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)        
    def clear(self):
